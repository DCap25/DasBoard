    ...
    if (error) {
      ...
      if (error.message?.includes('User already registered')) {
        ...
        if (session?.user?.email === formData.email) {
          ...
          console.log('[SIMPLE_SIGNUP] No matching session found - this might be a Supabase caching issue');
          ...
          if (signInData.user && !signInError) {
            ...
            return {
              success: false,
              message: 'There was an issue with your account. Please contact support or try with a different email.',
            };
          }
        }
      }
