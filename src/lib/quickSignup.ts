  ...
  try {
    ...
    if (existingSession?.user?.email === formData.email) {
      ...
      return {
        ...
        redirectTo: formData.accountType === 'single-finance' 
    ...
    console.log('[QUICK_SIGNUP] Authentication timeout after 8 seconds - checking one more time...');
    ...
    return {
      success: false,
      message: 'Signup is taking longer than expected. Please try signing in or wait a moment and refresh.',
    };
