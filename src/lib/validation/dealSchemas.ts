...
const dateString = z
  ...
  .refine(date => {
    ...
    return (
      ...
      parsed <= new Date('2030-12-31')
    );
  }, 'Date must be between 2020 and 2030');

// Lender validation
const lender = z.string().trim().max(100, 'Lender name too long').optional();

// Notes validation
const notes = z.string().trim().max(1000, 'Notes too long').optional();

/**
 * Single Finance Deal Log Form Validation Schema
 */
export const singleFinanceDealSchema = z
  .object({
    // Basic deal information
    dealNumber: dealNumber,
    stockNumber: stockNumber,
    vinLast8: vinLast8,
    vehicleType: z.enum(['N', 'U', 'C'], {
      errorMap: () => ({ message: 'Must select New, Used, or CPO' }),
    }),
    customerName: customerName,
    vehicleDescription: vehicleDescription,
    dealType: z.enum(['Finance', 'Lease', 'Cash'], {
      errorMap: () => ({ message: 'Must select a deal type' }),
    }),
    saleDate: dateString,
