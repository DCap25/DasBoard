...
export default function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { submitSimplifiedSignup } = await import('../../lib/signupService');

    try {
      const response = await submitSimplifiedSignup(formData);

      if (response.success) {
        // Show success message
        alert(response.message);

        // Redirect based on account type and next step
        if (response.data?.nextStep === 'email_verification') {
          // Single finance manager - direct to verification flow
          navigate('/auth?message=Please check your email to verify your account');
        } else if (response.data?.nextStep === 'payment_setup') {
          // Dealer groups - redirect to payment setup
          navigate('/pricing?signup=true');
        } else {
          // Default - back to auth
          navigate('/auth?message=Registration submitted successfully');
        }
      } else {
        // Show error message
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="bg-gray-800 rounded-xl p-8 border border-gray-700 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full bg-blue-600 hover:bg-blue-500 text-white py-4 px-6 rounded-lg text-lg font-semibold transition-colors duration-200 flex items-center justify-center hover:shadow-xl hover:shadow-blue-500/25"
            >
              Create Account
