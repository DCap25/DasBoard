...
export const SalespersonDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 4. Pay plan
      const { data: payPlanData } = await getPayPlan(userProfile.role_id, userProfile.dealership_id);
  ...
  const backEndGross = userDeals.reduce((sum, d) => sum + ((d.vsc_profit || 0) + (d.ppm_profit || 0) + (d.tire_wheel_profit || 0) + (d.paint_fabric_profit || 0) + (d.other_profit || 0)), 0);
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="w-56 bg-app-light dark:bg-app-dark border-r p-4 flex flex-col gap-4">
        <Button variant="secondary" onClick={() => navigate('/stats')}>Stats</Button>
        <Button variant="secondary" onClick={() => navigate('/pay')}>Pay</Button>
        <Button variant="secondary" onClick={() => navigate('/goals')}>Goals</Button>
        <Button variant="secondary" onClick={() => navigate('/schedule')}>Schedule</Button>
      </aside>
      {/* Main Content */}
      <main className="flex-1 p-8 space-y-8">
        <h1 className="text-3xl font-bold mb-4">Welcome, {user?.first_name}!</h1>
        <SalesLeaderboard leaderboard={leaderboard} />
        <StatsCards dealsCount={dealsCount} frontEndGross={frontEndGross} backEndGross={backEndGross} />
